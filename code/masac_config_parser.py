...
method = 'MAPPO_Cen_nMA' 
n_cluster = int(method.split('cluster')[1].split('_')[0]) if 'cluster' in method else 1
print('n_cluster: ', n_cluster)
st_iter_id = 0 # start iteration id. The paper test two, 0 and 2, for Four Deployments
max_FL_RSU = 12
...
def parse_arguments_for_MASAC():
    ...
    parser.add_argument('--tensorboard_writer_file', type=str, help='tensorboard_writer_file', default=f"../results/runs3/{method}")
    parser.add_argument('--saved_path', type=str, help='saved_path', default=f"../results/models/{method}_models")
    # saved models, in results
    parser.add_argument('--UAV_path_file', type=str, help='UAV_path_file', default="../data/uav_trace_dataset_results.pkl")
