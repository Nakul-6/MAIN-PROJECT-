...
class FL_PPO_Agent(Base_Algorithm_Module):
    ...
    def __init__(self, env, args):
        ...
        file_path = PROJECT_DIR / "data" / "max_speed_map_grid.pkl"

        with open(file_path, "rb") as f:
            max_speed_map_grid = pickle.load(f)
        # ./images/length_map_grid.pkl load map列表
        with open('../data/length_map_grid.pkl', 'rb') as f:
    ...
    def update_agents(self, cnt):
        ...
        for rsu_id in range(self.n_RSU):
            ...
            for t in reversed(range(len(reward))):
                ...
                if advantages_dict.get(last_agent_id) is None:
                    advantages_dict[last_agent_id] = []
                    obs_dict[last_agent_id] = []
                    other_agents_obs_dict[last_agent_id] = []
                    action_dict[last_agent_id] = []
                    target_value_dict[last_agent_id] = []
                advantages_dict[last_agent_id].append(last_adv)
                obs_dict[last_agent_id].append(obs[t])
                other_agents_obs_dict[last_agent_id].append(other_agents_obs[t])
                action_dict[last_agent_id].append(action[t])
                target_value_dict[last_agent_id].append(target_value[t])

            ppo_actor_dict = {}
            ppo_critic_dict = {}
            regional_exp_num = {}
            for key in advantages_dict.keys():
                # 对应每一个区域内的agent,都有一个actor和critic,复制一份
                ppo_actor_dict[key] = copy.deepcopy(ppo_actor)
                ppo_critic_dict[key] = copy.deepcopy(ppo_critic)
                obs_dict[key] = torch.stack(obs_dict[key])
                other_agents_obs_dict[key] = torch.stack(other_agents_obs_dict[key])
                advantages_dict[key] = torch.stack(advantages_dict[key])
                action_dict[key] = torch.stack(action_dict[key])
                target_value_dict[key] = torch.stack(target_value_dict[key])
                advantage = advantages_dict[key].to(self.device).detach()
                obs = obs_dict[key].to(self.device).detach()
                other_agents_obs = other_agents_obs_dict[key].to(self.device).detach()
                action = action_dict[key].to(self.device)
                target_value = target_value_dict[key].to(self.device)
                regional_exp_num[key] = len(advantages_dict[key])
                
                old_action_log_probs = torch.log(1e-5+ppo_actor_dict[key](obs, other_agents_obs).gather(1, action.unsqueeze(-1))).detach()
                train_actor_loss = 0
                train_critic_loss = 0
                entropy = 0
                for _ in range(self.args.train_n_episode):
                    action_prob = ppo_actor_dict[key](obs, other_agents_obs)
                    obs_encoded = ppo_actor_dict[key].state_encoder(obs).detach()
                    log_probs = torch.log(action_prob.gather(1, action.unsqueeze(-1))+1e-5)
                    entropy += -(action_prob * torch.log(action_prob+1e-5)).sum(dim=-1).mean()
                    ...
                    critic_loss = torch.mean(F.mse_loss(ppo_critic_dict[key](obs_encoded, other_agents_obs), target_value.detach())) 
                    ppo_actor_dict[key].opt_zero_grad()
                    actor_loss.backward()
                    ppo_actor_dict[key].opt_step()
                    ppo_critic_dict[key].opt_zero_grad()
                    critic_loss.backward()
                    ppo_critic_dict[key].opt_step()
                    train_actor_loss += actor_loss.item()
                    train_critic_loss += critic_loss.item()
            # 对每一个区域内的agent进行FedAvg,取均值

            # 初始化全局actor和critic的参数为0
            for param in ppo_actor.parameters():
                param.data.zero_()
            for param in ppo_critic.parameters():
                param.data.zero_()

            total_exp_num = sum(regional_exp_num.values())

            # 对每个区域的actor和critic进行加权求和
            for key in ppo_actor_dict.keys():
                weight = regional_exp_num[key] / max(1, total_exp_num)
                for global_param, local_param in zip(ppo_actor.parameters(), ppo_actor_dict[key].parameters()):
                    global_param.data += weight * local_param.data
                for global_param, local_param in zip(ppo_critic.parameters(), ppo_critic_dict[key].parameters()):
                    global_param.data += weight * local_param.data
